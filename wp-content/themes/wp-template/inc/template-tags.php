...
if ( ! function_exists( 'wp_bootstrap_4_posted_on' ) ) :
	...
	function wp_bootstrap_4_posted_on() {
		...
		$posted_on = sprintf(
			...
			esc_html_x( 'Размещено %s', 'post date', 'wp-bootstrap-4' ),
			'<a href="' . esc_url( get_permalink() ) . '" rel="bookmark">' . $time_string . '</a>'
		);

		$byline = sprintf(
			/* translators: %s: post author. */
			esc_html_x( 'хозяин %s', 'post author', 'wp-bootstrap-4' ),
			...
			comments_popup_link(
				sprintf(
					wp_kses(
						...
						__( 'Высказаться в комментариях<span class="screen-reader-text"> on %s</span>', 'wp-bootstrap-4' ),
						array(
							'span' => array(
								'class' => array(),
							),
		...
		edit_post_link(
			sprintf(
				wp_kses(
					...
					__( 'Редактировать <span class="screen-reader-text">%s</span>', 'wp-bootstrap-4' ),
					array(
						'span' => array(
							'class' => array(),
						),
